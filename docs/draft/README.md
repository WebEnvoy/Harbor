# Harbor 规划草稿

本目录保存 Harbor 当前阶段的规划草稿。

这些文档用于记录架构方向、浏览器 driver、Profile / Execution Identity、Evidence 和 Runtime Capability Facts 的设计候选，不代表已经定稿的 spec、PRD 或实现承诺。稳定契约应在后续具备 Schema、测试和迁移说明后单独标注。

## 文档

- [架构](architecture.md)
- [Browser Drivers](browser-drivers.md)
- [Evidence Store](evidence-store.md)
- [Profile 与 Execution Identity 模型](profile-identity-model.md)
- [Runtime Capability Facts](runtime-capability-facts.md)
