...
同样，Harbor 不应输出 normalized result、collection item、comment item、dataset record 或站点业务字段映射。这些属于 Lode 输出契约和 WebEnvoy Core 的 public result envelope。
...
- 数据归一化结果；
- 站点业务字段 Schema；
- collection / comment / dataset 公共结果模型。
