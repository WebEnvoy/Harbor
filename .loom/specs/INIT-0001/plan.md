...
- Deliver a carrier-only bootstrap repair for INIT-0001 so Harbor Stage 5 runtime/evidence-ref PRs can attach to a consumable Loom fact chain.
- Defer all product behavior and Stage 5 runtime/session/evidence implementation details to downstream real Work Item PRs.
...
- Exit condition: hosted `loom-pr-merge-gate` passes for Harbor bootstrap PR #130.

## Constraints

- Architectural or governance constraints: INIT-0001 remains bootstrap-only and must not carry Harbor runtime/session/evidence semantics.
- Workspace / rollout constraints: This branch only supports Stage 5 first-batch dependency setup and does not enter Stage 6.
- Purity or scope constraints: No credential, runtime profile storage, raw DOM, raw network, session truth, or evidence payload truth may be introduced here.
...
- Manual checks: PR body metadata readback for PR #130.
...
- Cases that are intentionally not automated: Runtime/evidence-ref behavior is intentionally validated in downstream Harbor implementation PRs.
- How failing tests or equivalent checks will be introduced before implementation: Hosted gate failure log from PR #130 is the failing check that drives this repair.
...
- Blocking inputs: Current PR #130 head SHA and hosted gate log.
