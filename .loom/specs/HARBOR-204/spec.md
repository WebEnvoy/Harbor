...
- Success experience: Harbor Runtime API stores local identity environments in memory with optional JSON persistence, exposes list/create/import plus per-ref read/update/delete HTTP routes, returns public status and redacted refs, opens a Runtime Session by managed environment ref, and rejects raw password, cookie, token, raw storage, raw profile, and raw endpoint material at input boundaries.
...
- consumer boundary: Harbor Runtime API consumers receive public status/redacted refs through HTTP and internal Runtime API tests consume managed facts only.
